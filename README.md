...
The first command will build the application. The second command will package and deploy your application to AWS, with a series of prompts:
...
```bash
spune$ sam logs -n SpuneFunction --stack-name spune --tail
```
...
```bash
spune$ pip install -r tests/requirements.txt --user
# unit test
spune$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
spune$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name spune
```
